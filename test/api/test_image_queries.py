...
def test_post_image_query_edge_only_mode_raises_runtime_error(test_client: TestClient):
    # Mock the app state to simulate edge-only mode
    with edge_app.dependency_overrides.get_app_state as mock_app_state:
        mock_app_state.edge_config.detectors.get.return_value = MagicMock(edge_only=True)
        mock_app_state.edge_inference_manager.inference_is_available.return_value = False

        response = test_client.post(url, json={"detector_id": "det_123"})
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert "Edge-only mode is enabled" in response.text
