...
from app.metrics.iqactivity import FilesystemActivityTrackingHelper, record_activity, clear_old_activity_files
...
def test_activity_tracking():
    ...
    with patch("app.metrics.iqactivity.datetime.now") as mock_now:
        mock_now.return_value = datetime(2025, 4, 3, 12, 0, 0)
        ...
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"iqs_2025-04-03_12").exists()
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"iqs_2025-04-03_12").read_text() == "1"
        assert Path(tmp_base_dir, "last_iq").exists()
        first_iq_time = _test_tracker.get_last_file_activity("last_iq")
        assert first_iq_time is not None
        assert Path(tmp_base_dir, "iqs").exists()
        assert Path(tmp_base_dir, "iqs").read_text() == "1"
        assert Path(tmp_base_dir, f"iqs_2025-04-03_12").exists()
        assert Path(tmp_base_dir, f"iqs_2025-04-03_12").read_text() == "1"

        # Record another IQ, make sure all files are updated correctly
        mock_now.return_value = datetime(2025, 4, 3, 12, 0, 1)
        record_activity("det_recordactivitytest", "iqs")
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", "iqs").read_text() == "2"
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"iqs_2025-04-03_12").read_text() == "2"
        second_iq_time = _test_tracker.get_last_file_activity("last_iq")
        assert second_iq_time is not None
        assert second_iq_time > first_iq_time
        assert Path(tmp_base_dir, "iqs").read_text() == "2"
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", "iqs").read_text() == "2"
        assert Path(tmp_base_dir, f"iqs_2025-04-03_12").read_text() == "2"
        ...
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"escalations_2025-04-03_12").exists()
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"escalations_2025-04-03_12").read_text() == "1"
        ...
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"audits_2025-04-03_12").exists()
        assert Path(tmp_base_dir, "detectors", "det_recordactivitytest", f"audits_2025-04-03_12").read_text() == "1"
...
def test_clear_old_activity_files():
    ...
    with patch("app.metrics.iqactivity.datetime.now") as mock_now:
        mock_now.return_value = datetime(2025, 4, 3, 12, 30, 0)
    ...
    assert not Path(tmp_base_dir, "iqs_2025-04-03_08").exists()
