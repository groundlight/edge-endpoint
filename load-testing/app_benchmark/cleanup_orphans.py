"""Standalone CLI to delete orphan cloud-side detectors after a SIGKILL leak.
...
def _parse_age(s: str | None) -> timedelta | None:
    ...
    if s[-1] in units:
        try:
            ...
            pass
    raise ValueError(f"could not parse --older-than: {s!r} (try '1h', '30m', '2d')")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--older-than", default=None, help="Only delete if at least this old (e.g. '1h', '2d').")
    parser.add_argument("--cloud-endpoint", default=None, help="Override Groundlight cloud endpoint URL.")
    ...
    if not args.prefix or len(args.prefix) < _MIN_PREFIX_LEN:
        logger.error("--prefix must be ≥%d chars; refusing to act on %r", _MIN_PREFIX_LEN, args.prefix)
        return 2
    try:
    ...
    logger.info(
        "%s %d detector(s) matching prefix %r:",
