...
When the EE is not running in minimal mode, each detector will have 2 `inferencemodel` pods, one for the primary model and one for the out of domain detection (OODD) model.
In minimal mode, only a single `inferencemodel` pod is used, which performs both primary and OODD inference. 
