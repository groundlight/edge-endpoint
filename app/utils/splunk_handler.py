...
class SplunkHECHandler(logging.Handler):
    ...
    def __init__(
        ...
        self.session.headers.update({"Authorization": f"Splunk {self.hec_token}", "Content-Type": "application/json"})
    ...
    def _test_connection(self, retry_count=10, retry_delay=10):
        ...
        for attempt in range(retry_count):
            try:
                ...
                response = self.session.get(health_url, timeout=5, verify=self.verify_ssl)
        ...
        while not self.stop_event.is_set():
            ...
            if not self.connected:
                ...
                if time_since_last >= reconnect_interval:
                    print("Attempting to reconnect to Splunk HEC...")
                    if self._test_connection(retry_count=1):
                        print("Reconnected to Splunk HEC successfully")
        ...
        for attempt in range(self.retry_count):
            try:
                response = self.session.post(
                    f"{self.hec_url}/services/collector/event", data=payload, timeout=10, verify=self.verify_ssl
                )
                ...
                if response.status_code == 200:
                    ...
                    print("Splunk HEC authentication failed. Check token.")
                    self.connected = False
                    return
                else:
...
def create_splunk_logger(
    name: str, detector_id: Optional[str] = None, component: Optional[str] = None, level: int = logging.INFO
) -> logging.Logger:
    """
