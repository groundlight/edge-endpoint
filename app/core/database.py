...
class DatabaseManager:
    ...
    def create_or_update_inference_deployment_record(self, deployment: Dict[str, str]) -> None:
        ...
        try:
            ...
            if "deployment_name" not in str(e.orig):
                raise e
    ...
    def _handle_existing_detector(self, deployment: Dict[str, str]) -> None:
        ...
        :param deployment: A dictionary containing the deployment details.
        """
        logger.debug(f"Deployment name {deployment['deployment_name']} already exists in the database.")
        detectors = self.get_inference_deployment_records(deployment_name=deployment["deployment_name"])
        if len(detectors) != 1:
    ...
    def update_inference_deployment_record(self, deployment_name: str, fields_to_update: Dict[str, Any]):
        ...
        """
        with self.session_maker() as session:
            query = select(InferenceDeployment).filter_by(deployment_name=deployment_name)
