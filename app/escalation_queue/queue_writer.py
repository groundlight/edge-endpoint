...
class QueueWriter:
    ...
    def write_escalation(self, escalation_info: EscalationInfo) -> bool:
        ...
        Will write to the last file path if it exists and has not exceeded the maximum length. Otherwise will create
        a new file to write the escalation to.
    ...
    def _write_to_path(self, path_to_write_to: Path, data: EscalationInfo) -> bool:
        """
        Writes the provided data to the provided path.

        Returns True if the write succeeds and False otherwise.
        """
        try:
            with path_to_write_to.open(mode="a") as writing_fd:  # TODO open once per file, instead of once per write?
