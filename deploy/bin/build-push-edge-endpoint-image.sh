...
# This script builds and pushes the non-FIPS edge-endpoint Docker image to GL_Public ECR.
#
# Usage:
#   ./build-push-edge-endpoint-image.sh
#
# Publishes only to 767397850842 (repo edge-endpoint). FIPS images go to the Axon
# master ECR via build-push-edge-endpoint-image-fips.sh. This job never logs into
# 794562053834: infosec policy is that Axon ECR holds FIPS images only.
...
# Authenticate docker to GL_Public ECR only.
...
echo "${GL_PUBLIC_IMG}"
echo "Non-FIPS build: not publishing to the Axon edge master ECR, which by policy only receives FIPS-validated images."
