...
const PopoverTarget = factory.factory((props, ref) => {
  ...
  const targetRef = hooks.useMergedRef(ctx.reference, getRefProp.getRefProp(child), ref);
  ...
  return React.cloneElement(child, {
    ...
    ...accessibleProps,
    ...ctx.targetProps,
    className: cx__default.default(ctx.targetProps.className, forwardedProps.className, childProps.className),
    [refProp]: targetRef,
    ...!ctx.controlled ? {
      onClick: () => {
        ctx.onToggle();
